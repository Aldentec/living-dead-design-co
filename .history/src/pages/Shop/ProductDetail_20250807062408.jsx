...
export default function ProductDetail() {
  ...
  const formatPrice = (price, salePrice) => {
    ...
    if (currentSalePrice && currentSalePrice < currentPrice) {
      return (
        <>
          <span className="h3 text-danger fw-bold">${currentSalePrice.toFixed(2)}</span>
          <span className="text-muted text-decoration-line-through ms-3 h5">${currentPrice.toFixed(2)}</span>
        </>
      );
    }
    return <span className="h3 fw-bold text-light">${currentPrice.toFixed(2)}</span>;
  };
                    ...
                    <div className="d-flex flex-wrap gap-2">
                      {product.variants.map((variant, index) => (
                        <Button
                          ...
                          className="d-flex flex-column align-items-center variant-button"
                        >
                          <span>{variant.value}</span>
                          {variant.price && variant.price !== product.price && (
                            <small className="mt-1 fw-bold">${variant.price.toFixed(2)}</small>
                          )}
                          {variant.quantity <= 0 && <small className="mt-1 text-danger">(Out of Stock)</small>}
                          {variant.quantity <= 5 && variant.quantity > 0 && (
                            <small className="mt-1 text-warning">({variant.quantity} left)</small>
                          )}
                        </Button>
                      ))}
                    </div>
                    {selectedVariant && (
                      <div className="mt-3 p-2 bg-dark rounded">
                        <small className="text-muted d-block">
                          <strong>Selected:</strong> {selectedVariant.option} - {selectedVariant.value}
                        </small>
                        {selectedVariant.price && selectedVariant.price !== product.price && (
