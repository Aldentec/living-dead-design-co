...
export default function Home() {
  ...
  const fetchFeaturedProducts = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${LAMBDA_URL}/items`);
  ...
  const handleNewsletterSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${LAMBDA_URL}/newsletter/subscribe`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        setMessage('Successfully subscribed! Welcome to the dark side.');
        setMessageType('success');
        setEmail(''); // Clear form
      } else {
        if (response.status === 409) {
          setMessage('You\'re already part of our dark family!');
          setMessageType('error');
        } else {
